...
class Vgg19:
    def __init__(self, vgg19_npy_path=None):
        if vgg19_npy_path is None:
            path = inspect.getfile(Vgg19)  #返回对象的文件名
            path = os.path.abspath(os.path.join(path, os.pardir))  # os.pardir获取当前目录的父目录字符串名  # 返回path规范化的绝对路径
    ...
    def conv_layer(self, bottom, name):
        with tf.variable_scope(name):   #可以让变量有相同的命名
