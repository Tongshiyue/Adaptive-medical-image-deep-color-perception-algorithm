...
def print_loss(args, loss_content, loss_styles_list,loss_tv,loss_affine,styleswap_loss,overall_loss, output_image,loss_style):
    global iter_count, min_loss, best_image, loss_contents
    if iter_count <= 500:
        loss_contents.append(loss_affine)
    elif iter_count > 1000 and iter_count <= 1500:
        loss_contents.append(loss_affine)

    if iter_count % args.print_iter == 0:
        # print('Iteration {} / {}\n\t'.format(iter_count, args.max_iter))
...
def y_loss(content_image,output_image,weight=1):
    ...
    # return (0.8*tf.reduce_mean(tf.norm(tf.subtract(content_h,output_h),ord=1))+0.2* tf.reduce_mean(tf.squared_difference(content_h, output_h))) * weight, output_h,content_h
  ...
  #   styleswap_loss += tf.reduce_mean(tf.squared_difference(styleswap, content_conv2_1_var)) * args.swapweight
    ...
    if args.lbfgs:
        if not Matting:
            ...
            overall_loss = loss_tv + loss_affine + loss_style + styleswap_loss + loss_content
        ...
        global min_loss, best_image, iter_count, loss_contents
        best_result = copy.deepcopy(best_image)
        min_loss, best_image = float("inf"), None
        loss = copy.deepcopy(loss_contents)
        loss_contents = []
        iter_count = 0

        sess.close()
        return best_result,loss
    else:
